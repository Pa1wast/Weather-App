...
async function updateDisplay() {
    ...
    if (_modelJs.state.curUnit === "\xb0C") {
        ...
        (0, _viewJsDefault.default).updateHighlightCards(_modelJs.state.weatherData, "\xb0C");
    } else {
        (0, _viewJsDefault.default).updateTemp(_modelJs.state.weatherData.tempF, "\xb0F");
        (0, _viewJsDefault.default).updateWeatherCards(_modelJs.state.weatherData.forecast.forecastHour, "\xb0F");
        (0, _viewJsDefault.default).updateHighlightCards(_modelJs.state.weatherData, "\xb0F");
    }
    (0, _viewJsDefault.default).updateCurText(_modelJs.state.weatherData.forecast.text, _modelJs.state.weatherData.cityName);
}
updateDisplay();
function controlSwitchUnit(unit) {
    _modelJs.state.curUnit = unit;
    updateDisplay();
}
async function controlSearch(cityName) {
    try {
        await _modelJs.getData(cityName);
        updateDisplay();
    } catch (err) {
        (0, _viewJsDefault.default).ErrorMessage();
    }
}
function init() {
    (0, _viewJsDefault.default).addSwitchUnitHandler(controlSwitchUnit);
    (0, _viewJsDefault.default).addSearchHandler(controlSearch);
}
init();

},{"./model.js":"Y4A21","./view/view.js":"4wVyX","./helper.js":"lVRAz","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"Y4A21":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "state", ()=>state);
parcelHelpers.export(exports, "getDataFromUser", ()=>getDataFromUser);
...
function getDate() {
    setInterval(()=>{
        ...
        curTime.innerHTML = `${state.date.time.hour.toString()}:${state.date.time.minute < 10 ? "0" + state.date.time.minute.toString() : state.date.time.minute.toString()}:${state.date.time.seconds < 10 ? "0" + state.date.time.seconds.toString() : state.date.time.seconds.toString()}`;
...
const userLocation = new Promise((resolve, reject)=>{
    if (navigator.geolocation) navigator.geolocation.getCurrentPosition((position)=>{
        resolve((0, _helperJs.getCoords)(position, state));
    });
    else reject("Can't access your position!");
});
async function getDataFromUser() {
    await userLocation;
    const data = await (0, _helperJs.fetchApi)(state.position.latitude, state.position.longitude, (0, _helperJs.API_KEY), "forecast");
    updateState(data);
}
async function getLocation(cityName) {
    try {
        ...
        state.weatherData.cityName = data[0].display_name.split(" ")[0].replace(",", "");
    } catch (err) {
        throw err;
    }
}
...
const API_KEY = "ecdfbd386d04459d8a9100346231710";
...
exports.default = new WeatherDisplay();

},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["kYpTN","aenu9"], "aenu9", "parcelRequire2a28")
